  ...
  router.post("/login", (req, res) => {
    ...
    User.findOne({ username })
      .then((possibleUser) => {
        ...
        res.redirect("/profile"); // <-- rethinking how we reach the user's profile cuz only the current user is allowed to see the own profile, not the one from other people by just entering /user/random-name
      })
      .catch((err) => {
